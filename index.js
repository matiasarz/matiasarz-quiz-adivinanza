...
const containerGame = document.querySelector(".contenedor");
...
const gameOver = ()=> {
  ...
  contadorPuntos = 0;
  containerGame.appendChild(cloneGameOver);
  containerGame.querySelector(".menu").addEventListener("click",menu)
}
const ramdonNumbers = ()=> {
...
const afterPlay = (id1,id2)=> {
  ...
  if (cloneGame.hasChildNodes()) {
    cloneGame.querySelector(".pregunta-1 img").src = `${personas[id1].imagen}`;
    cloneGame.querySelector(".pregunta-2 img").src = `${personas[id2].imagen}`;
    
    cloneGame.querySelector(".name-one .full-name").textContent = `"${personas[id1].nombre}"`;
    cloneGame.querySelector(".name-one .has").textContent = `has`;

    cloneGame.querySelector(".name-one .cash").textContent = personas[id1].patrimonio.toLocaleString('en');

    cloneGame.querySelector(".options .name-two").textContent = `"${personas[id2].nombre}"`;
    cloneGame.querySelector(".options .left").textContent = `"${personas[id1].nombre}"`;

    cloneGame.querySelector(".name-one .h3-3").textContent = localStorage.getItem("puntos") === null ? `High Score: ` + 0 : `High Score: ${localStorage.getItem("puntos")}`;
  ...
  containerGame.appendChild(contenedor2);
}
...
const menu = ()=> {
  ...
  containerGame.appendChild(contenedor1);
}
